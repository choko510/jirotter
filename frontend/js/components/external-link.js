        ...
        contentArea.innerHTML = `
            <div class="external-link-page" style="max-width: 640px; margin: 40px auto; padding: 32px; background: #ffffff; border-radius: 16px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.08);">
                <div style="text-align: center; margin-bottom: 24px;">
                    <i class="fas fa-external-link-alt" style="font-size: 36px; color: var(--color-primary);"></i>
                ...
                <div style="display: flex; gap: 12px; flex-wrap: wrap; justify-content: center;">
                    ...
                    <button id="externalLinkProceed" ${isValidUrl ? '' : 'disabled'} style="flex: 1 1 200px; padding: 12px 16px; border-radius: 999px; border: none; background: ${isValidUrl ? 'var(--color-primary)' : '#d0c2ae'}; color: ${isValidUrl ? '#1a1a1a' : '#666'}; font-weight: bold; cursor: ${isValidUrl ? 'pointer' : 'not-allowed'};">
                        外部サイトへ移動
                    </button>
                </div>
            </div>
